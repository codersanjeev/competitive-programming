...
class Solution {
	func maxPerformance(_ n: Int, _ speed: [Int], _ efficiency: [Int], _ k: Int) -> Int {
		var (ans, sum, mod) = (Int64(0), Int64(0), Int64(1000000007))
		var teamMembers = [[Int]]()
		for i in speed.indices {
			teamMembers.append([efficiency[i], speed[i]])
		}
		teamMembers = teamMembers.sorted { $0[0] > $1[0] }
		var heap = PriorityQueue<Int>(sort: <)
		teamMembers.forEach { member in
			heap.enqueue(member[1])
			sum += Int64(member[1])
			if heap.count > k {
			...
			ans = max(ans, sum * Int64(member[0]))
		}
		return Int(ans % mod)
	}
}
